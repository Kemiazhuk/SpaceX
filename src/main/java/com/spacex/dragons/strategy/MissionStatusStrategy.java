...
public class MissionStatusStrategy {

    public MissionStatus calculateStatus(Mission mission) {
        if (mission.getMissionStatus() == MissionStatus.ENDED) return MissionStatus.ENDED;
        if (mission.getRockets().isEmpty()) return MissionStatus.SCHEDULED;
